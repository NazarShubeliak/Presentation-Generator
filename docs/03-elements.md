...
| Element | Fixed / Variable | Detail |
|---|---|---|
| Theme-world caption(s) | **Variable** | 1 caption per theme, **always a separate caption per theme even on a 2-up slide** (designer confirmed 2026-08-20, `docs/open-questions.md` #19 — not one combined caption spanning both). **Limit: 25 characters** (the "55 for a combined caption" contingency from earlier drafts no longer applies). Typography resolved: `HelveticaNeue-CondensedB`, 27.41pt, `#f2f0ee`, top-left — the Basel single-theme style is now canonical for every instance of this page type, real text placeholder (not baked into the image). |
...
**Caption styling resolved by designer (2026-08-20, see `docs/open-questions.md` #19).** Basel's style (top-left, `HelveticaNeue-CondensedB` 27.41pt, `#f2f0ee`) is canonical for all positions and both 1-up/2-up layouts; Halle's and Magdeburg's bottom-position styles were the outliers, not valid alternate treatments. No open question remains for this page type besides the 2-up photo split ratio (PowerPoint-layout decision, not a designer question).
...
| Background-choice label shown in the kiosk screenshot | **Variable, confirmed separate field 2026-08-20** (`docs/open-questions.md` #21) | Not baked into `IMG_KIOSK_SCREENSHOT` after all — a real text field, `TXT_KIOSK_BACKGROUND_LABEL`. Filled from a brief listing characters, places, and defined landmarks per market, supplied ahead of generation. |
| Output-card example photo(s) | **Variable, 2 separate photos, vertical stack** (`docs/open-questions.md` #20, confirmed 2026-08-20) | Count/arrangement previously inconsistent across decks: 2 large cards (Basel/Freiburg/Magdeburg, one flattened combined image at ratio ≈1.78), several small thumbnails (Erzgebirgsdorf), 4 medium cards (Halle). Designer confirmed the intended architecture is **2 independent photos, stacked one above the other** (not side by side, as the source decks' flattened image suggested) — same per-image composition mechanism as `PAGE_06_LOCAL_MOTIFS`'s motif cells. Individual per-card aspect ratio still needs a PowerPoint-layout decision — the old 1.78 combined ratio was for a horizontal pairing and doesn't apply to a vertical stack. |
...
**Headline/subheadline rotation scheme rewritten by the designer, 2026-08-20
(`docs/open-questions.md` #22) — supersedes everything below the table.**
The entire previous rotation ("Jede Karte ist ein Unikat...", "Das nehmen
Ihre Besucher mit...", "Ihre Charaktere. Ihre Geschichte.", investigated at
length across WP4/WP5/WP6 — see the historical open questions further down
...
| Column headers ("Fotokarte", "Hintergrund", "AR-Maske") | **Fixed — 3 separate labels, never varies** (revised 2026-08-27, see `docs/05-template.md`) | Identical everywhere, all 5 decks' first-table instances show all 3 as separate positioned labels, one per column. The originally-planned 2-box split (`_1`/`_23` combined) turned out unbuildable — the combined box can't fit "Hintergrund" + "AR-Maske" on one line without wrapping into column 3, found while sanity-checking the built template against real Basel content. Typography: `MyriadPro-Regular`, 11.87pt, `#231f20`. |
...
superseded 2026-08-20 — kept for context on how the final scheme was
reached, not needed to build the template)</summary>

1. **The headline-rotation rule doesn't match measurement.** The designer's original stated rule (`docs/02-page-types.md`) was: world 1 → "Jede Karte ist ein Unikat" + stats, world 2 → "Das nehmen Ihre Besucher mit", world 3+ → "Ihre Charaktere." But the *first* standalone motif table in **every** multi-theme deck measured (Basel, Halle, Magdeburg) actually used "Das nehmen..." (the stated world-2 pattern), and the *second* table used "Jede Karte..." (the stated world-1 pattern) — consistently reversed. First resolved 2026-08-18 (`docs/open-questions.md` #10) as "the reference decks have the bug, not the rule" — then the whole rule was replaced outright on 2026-08-20 (see table above), making this thread moot either way.
2. **The motif table is one flattened image per slide in the source, not 9 separate cell images.** Resolved by designer (2026-08-18, `docs/open-questions.md` #9): 9 separate per-cell images, composed by the generator. Still current — not superseded.
3. Not every theme-world's motifs are location-specific even within a single deck (Magdeburg's "Märchengasse" etc. read as generic). Never formally asked — low priority, doesn't block WP6.
4. **Intro paragraph wording, re-measured 2026-08-19 against all 5 decks** — found a bigger 3-way/4-way disagreement than the original designer answer (`docs/open-questions.md` #16) had covered, including a text at "position 3" never previously documented. Logged as `docs/open-questions.md` #22. **Superseded 2026-08-20** — the designer replaced the entire rotation scheme rather than picking among the measured variants; see the table above for the actual final wording.
5. **Stats callout wording, re-measured 2026-08-19** — found a second wording used by a majority of decks (3 of 5), the opposite of what `docs/open-questions.md` #7 had settled on. Logged as `docs/open-questions.md` #23. **Resolved 2026-08-20** — designer kept the original #7 answer unchanged; see the table above.
...
**Reopened 2026-08-19 while building `docs/05-template.md`, re-confirmed 2026-08-20** (`docs/open-questions.md` #24). Pulled the actual caption text from Halle's measurement data (the only deck where it's present): "Hallescher Weihnachtsmarkt" — this reads as the market's own name, not any of Halle's 5 real theme-world names. Asked the designer directly; **answer: the field is `TXT_THEME_WORLD_NAME`, confirmed** — same field/mechanism as `PAGE_03_THEME_SHOWCASE`/`PAGE_06_LOCAL_MOTIFS`, the #8 resolution stands. The Halle text itself is now understood as a **content mistake in that one reference deck** (wrong value entered for this field), not evidence the field is actually market-name — same category as this project's other confirmed reference-deck content bugs. Typography for this page type specifically: `HelveticaNeue-Medium`, 14.19pt, `#ffffff` fill / `#000000` stroke — different from `TXT_THEME_WORLD_NAME`'s styling on `PAGE_01`/`02`/`06` (`HelveticaNeue-CondensedB`, 14.26pt, `#231f20`), same field with page-type-specific styling, consistent with how other reused fields already work.
...
| Instagram mockup photo | **Variable** | Frame ratio: 1.33 in 3/5 decks, 1.31 (Erzgebirgsdorf), 1.03 (Halle, notably square-ish, an outlier). **Target 1.33.** |
...
**Caption re-checked 2026-08-19, confirmed by designer 2026-08-20** (`docs/open-questions.md` #25). No separate caption text object exists next to the mockup image in any of the 5 decks — confirmed this isn't a measurement gap: the market-name caption is **baked into the `IMG_INSTAGRAM_MOCKUP` photo asset itself** (part of the mocked-up phone screen), not something the template or generator overlays. No text field for this element.
